...
```bash
secretproxy         # start on port 9900
```
...
```bash
ANTHROPIC_BASE_URL=http://localhost:9900/anthropic claude
```
...
```bash
OPENAI_BASE_URL=http://localhost:9900/codex codex
```
...
secretproxy service status
secretproxy service logs
```

The proxy starts automatically on login and restarts on failure.
